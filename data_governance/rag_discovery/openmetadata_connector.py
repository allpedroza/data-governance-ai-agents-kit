# Dependencies managed via pyproject.toml — install with: pip install -e ".[rag]"
...
try:
    from data_governance.ports.catalog import MetadataCatalogPort
except ImportError:
    try:
        from ports.catalog import MetadataCatalogPort
    except ImportError:
        MetadataCatalogPort = object  # type: ignore[assignment,misc]
...
@dataclass
class OpenMetadataConnector(MetadataCatalogPort):  # type: ignore[misc]
    """Adapter that implements :class:`MetadataCatalogPort` for OpenMetadata."""
    ...
    def test_connection(self) -> bool:
        """Verify that the OpenMetadata server is reachable."""
        try:
