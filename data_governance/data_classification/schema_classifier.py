# Moved from data_governance/classification/data_classification_agent.py
# Retained here as the schema-only LGPD/GDPR classifier (no raw-data access).
...
class SchemaClassificationAgent:
    """Classify sensitive data using schema-level signals only.

    .. deprecated::
       Use ``SchemaClassificationAgent`` directly.  The old name
       ``DataClassificationAgent`` from the ``classification/`` package
...
# Backwards-compatible alias for imports that used the old module path
DataClassificationAgent = SchemaClassificationAgent
